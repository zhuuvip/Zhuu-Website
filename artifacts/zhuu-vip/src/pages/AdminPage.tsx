...
export default function AdminPage() {
  ...
  const loadWalletUsers = async (query = "") => {
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/admin/wallet/users?${params.toString()}`,
        {
          headers: await authHeaders(),
        }
      );
      ...
      if (!res.ok) {
        alert(data.error || "Gagal mengambil daftar member");
        return;
      }
  ...
  const selectWalletUser = (member: any) => {
    setSelectedWalletUser(member);
    setWalletUserId(member.id);
  };
        ...
        <div className="space-y-4">
          ...
          <div className="glass-card rounded-2xl p-5 space-y-4">
            ...
            {selectedWalletUser && (
              <div className="rounded-xl border border-cyan-400/20 bg-cyan-400/5 p-3">
                ...
                <div className="text-xs text-green-300 mt-2">
                  Saldo saat ini: Rp
                  {Number(selectedWalletUser.balance || 0).toLocaleString("id-ID")}
                </div>
              </div>
            )}
            ...
            <button
              ...
              disabled={walletAdjusting || !selectedWalletUser}
